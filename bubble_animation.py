...
def bubble_anim(image_with_bubble_filename, image_without_bubble_filename, bubble_position, bubble_diameter, t, output_filename):
  ...
  if(((bubble_position[0] + bubble_radius) > pano_image.size[0]) or ((bubble_position[0] - bubble_radius) < 0)):
    ...
    bubble_image = rotate_pano_image(bubble_image, bubble_position[0], move_to)
    pano_image = rotate_pano_image(pano_image, bubble_position[0], move_to)
    
    bubble_position = (bubble_position[0] - move_amount, bubble_position[1])
  
  bubble_rect = (bubble_position[0] - bubble_radius, bubble_position[1] - bubble_radius, bubble_position[0] + bubble_radius, bubble_position[1] + bubble_radius)
  bubble = bubble_image.crop(bubble_rect)
  
  mask = Image.new('1', bubble.size)
  draw = ImageDraw.Draw(mask)
  draw.ellipse((0,0,bubble.size[0],bubble.size[1]), fill = 'white', outline ='white')
  ...
  new_im.paste(bubble, (bubble_position[0] - int(bubble_size / 2), bubble_position[1] - int(bubble_size / 2)), mask=mask)
  
  if(need_to_shift_image):
    new_im = rotate_pano_image(new_im, bubble_position[0], bubble_position[0] + move_amount)
...
if __name__ == '__main__':
  ...
  for i in range(int(num_frames),1,-1):
    bubble_anim('./buddha/zf4_sv1p4_zc1p2/bubble_out_anim/pano_bubble.png','./buddha/zf4_sv1p4_zc1p2/bubble_out_anim/pano_nobubble.png', bubble_position, bubble_diam, float(i)/num_frames, "./buddha/zf4_sv1p4_zc1p2/bubble_out_anim/out_bubble_anim_" + str(int(num_frames - i)).zfill(3) + ".png")
