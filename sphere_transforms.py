...
def angles_from_pixel_coords(point, x_size=1000):
    ...
    return ((point[0] + 0.5) * 2 * pi / float(x_size),
            point[1] * pi / float(y_size - 1) - 0.5 * pi)


def pixel_coords_from_angles(point, x_size=1000):
    """map from (0, 2*pi) x (-pi/2, pi/2) rectangle to pixel coords"""
    y_size = x_size / 2  # assume equirectangular format
    return (point[0] * float(x_size) / (2 * pi) - 0.5,
            (point[1] + 0.5 * pi) * float(y_size - 1) / pi)
...
def get_pixel_colour(pt, s_im, x_size=1000):
    ...
    pt = clamp(pt, x_size)
    return vector(s_im[int(pt[0]), int(pt[1])])
...
def get_interpolated_pixel_colour(pt, s_im, x_size=1000):
    ...
    out_colour = (1-f)*((1-g)*get_pixel_colour([x, y], s_im, x_size=x_size) + g*get_pixel_colour([x, y+1], s_im, x_size=x_size)) \
        + f*((1-g)*get_pixel_colour([x+1, y], s_im, x_size=x_size) +
             g*get_pixel_colour([x+1, y+1], s_im, x_size=x_size))
...
def three_points_to_three_points_pixel_coords(p1,
                                              q1,
                                              r1,
                                              p2,
                                              q2,
    ...
    p1, q1, r1, p2, q2, r2 = [
        CP1_from_sphere(sphere_from_pixel_coords(point, x_size=x_size))
...
def apply_SL2C_elt_to_image(M,
    ...
    Minv = matrix2_inv(
        M
    )  # to push an image forwards by M, we pull the pixel coordinates of the output backwards
    ...
    for i in range(out_x_size):
        for j in range(out_y_size):
            ...
            o_im[i, j] = get_interpolated_pixel_colour(pt,
                                                       s_im,
                                                       x_size=in_x_size)
...
def generate_image_pt(i, j, in_x_size, out_x_size, M_rot, M_rot_inv, zoom_factor, zoom_loop_value, zoom_cutoff, s_im_A, s_im_B):
    pt = (i, j)
    pt = angles_from_pixel_coords(pt, x_size=out_x_size)
    pt = sphere_from_angles(pt)
    pt = CP1_from_sphere(pt)
    ...
    if (floor(recurse_value) >= 0.0):
        ...
        pt = complex(
            (pt.real + zoom_cutoff) % log(zoom_factor) - zoom_cutoff,
            pt.imag)
    elif (floor(recurse_value) == -2.0):
        pt = complex((pt.real + zoom_cutoff) % log(zoom_factor) -
                     zoom_cutoff - log(zoom_factor), pt.imag)
    elif (floor(recurse_value) == -3.0):
        pt = complex((pt.real + zoom_cutoff) % log(zoom_factor) -
                     zoom_cutoff - (log(zoom_factor) * 2), pt.imag)
    else:
        # this is "future spheres"
        pt = complex((pt.real + zoom_cutoff) % log(zoom_factor) -
                     zoom_cutoff - (log(zoom_factor) * 3.0), pt.imag)
    ...
    if (floor(recurse_value) >= 0):
        return get_interpolated_pixel_colour(
            pt, s_im_A, in_x_size)
    else:
        return get_interpolated_pixel_colour(
            pt, s_im_B, in_x_size)
...
def get_color(in_x_size, out_x_size, out_y_size, M_rot, M_rot_inv, zoom_factor, zoom_loop_value, zoom_cutoff, s_im_A, s_im_B):
    out_color = list()
    ...
    for _ in trange(1):
        ...
        for i in range(out_x_size):
            for j in range(out_y_size):
                args.append((i, j, in_x_size, out_x_size, M_rot, M_rot_inv, zoom_factor,
                            zoom_loop_value, zoom_cutoff, s_im_A, s_im_B))

        with Pool(processes=4) as pool:
            out_color = pool.starmap(generate_image_pt, args)
    return out_color
...
def generate_image(zoom_center_pixel_coords,
    ...
    loaded_A = source_image_A.load()
    s_im_A = np.zeros((in_x_size, in_y_size), dtype=tuple)
    for i in range(in_x_size):
        for j in range(in_y_size):
            s_im_A[i, j] = loaded_A[i, j]

    source_image_B = Image.open(source_image_filename_B)
    loaded_B = source_image_B.load()
    s_im_B = np.zeros((in_x_size, in_y_size), dtype=tuple)
    for i in range(in_x_size):
        for j in range(in_y_size):
            s_im_B[i, j] = loaded_B[i, j]
    ...
    out_color = get_color(in_x_size, out_x_size, out_y_size, M_rot, M_rot_inv, zoom_factor,
                          zoom_loop_value, zoom_cutoff, s_im_A, s_im_B)
    ...
    for i in range(out_x_size):
        for j in range(out_y_size):
            o_im[i, j] = out_color[i * out_y_size + j]
...
if __name__ == "__main__":
    ...
    for i in range(TOTAL):
        generate_image(
            ...
            tmp_file_name, 2048, i / TOTAL, "{}.png".format(i))

    # imgs = []
    # for i in range(TOTAL):
    #     imgs.append(imageio.imread("{}.png".format(i)))
