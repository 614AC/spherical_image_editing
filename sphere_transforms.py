...
def angles_from_pixel_coords(pts, size):
    ...
    """
    pts = np.asarray(
        pts, dtype=np.float64)  # turn into an ndarray if necessary
...
def inf_zero_one_to_triple(p, q, r):
    ...
    M = np.vstack((p, q)).T
    mu, lam = np.linalg.lstsq(M, r, rcond=None)[0]
...
def two_triples_to_SL(a1, b1, c1, a2, b2, c2):
    ...
    return np.linalg.lstsq(M1.T, M2.T, rcond=None)[0].T
...
def generate_image(zoom_center_pixel_coords,
    ...
    source_image_A = np.array(Image.open(
        source_image_filename_A), dtype=np.float32)
    s_im_A = np.atleast_3d(source_image_A)
    source_image_B = np.array(Image.open(
        source_image_filename_B), dtype=np.float32)
    s_im_B = np.atleast_3d(source_image_B)

    out_size = in_size = s_im_A.shape[:-1]

    M_rot = rotate_pixel_coords_p_to_q((zoom_center_pixel_coords[1], zoom_center_pixel_coords[0]), (0, 0),
                                       in_size)
    M_rot_inv = np.linalg.inv(M_rot)

    pts = np.indices(out_size).reshape((2, -1))
    pts = angles_from_pixel_coords(pts, out_size)
    pts = sphere_from_angles(pts)
    pts = CP1_from_sphere(pts)
    pts = np.dot(M_rot, pts)
    ...
    pts.real += np.log(zoom_factor) * zoom_loop_value

    recurse_value = (pts.real + zoom_cutoff) / np.log(zoom_factor)
    ...
    for i in range(pts.shape[0]):
        if (np.floor(recurse_value[i]) >= 0.0):
            # main and prev spheres => do nothing to pt
            someval = "do nothing further"
        else:
            pts[i] = complex((pts[i].real + zoom_cutoff) % np.log(zoom_factor) - zoom_cutoff - np.log(
                zoom_factor) * np.clip(-np.floor(recurse_value[i]) - 1, None, 3.0), pts[i].imag)

    pts = np.exp(pts)
    pts = np.vstack((pts, np.ones_like(pts)))
    pts = np.dot(M_rot_inv, pts)
    pts = sphere_from_CP1(pts)
    pts = angles_from_sphere(pts)
    pts = pixel_coords_from_angles(pts, in_size)
    pts = pts.reshape((2,) + out_size)

    interpolated_color_A = get_interpolated_pixel_color(pts, s_im_A, in_size)
    interpolated_color_B = get_interpolated_pixel_color(pts, s_im_B, in_size)

    o_im = np.zeros_like(interpolated_color_A)
    cond = np.floor(recurse_value) >= 0
    cond = cond.reshape(out_size)
    for i in range(cond.shape[0]):
        for j in range(cond.shape[1]):
            o_im[i][j] = interpolated_color_A[i][j] if cond[i][j] else interpolated_color_B[i][j]
    np.clip(o_im, 0, 255, o_im)
    o_im = o_im.astype(np.uint8)
    Image.fromarray(o_im).save(save_filename)
