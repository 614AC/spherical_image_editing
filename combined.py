...
def one_shot_with_zc_zf(pano_A_zoom_centre, pano_B_zoom_centre, pano_folder, pano_a_short_name, pano_b_short_name, zoom_cutoff, zoom_factor, bubble_out_diameter, bubble_in_diameter, outputsize, num_frames):
  print "Starting: " + pano_a_short_name + " to " + pano_b_short_name
  ...
  print pano_a_filename
  print pano_b_filename
  ...
  pano_A_filepath = pano_folder + pano_a_filename
  pano_B_filepath = pano_folder + pano_b_filename
  folder_name = pano_folder + 'zf_zc_one_shot_generation_exec/_zf_' + str(zoom_factor) + "_zc_" + str(zoom_cutoff) + "/" + pano_a_short_name + "_to_" + pano_b_short_name
  
  one_shot(folder_name, pano_A_filepath, pano_B_filepath, pano_A_zoom_centre, pano_B_zoom_centre, bubble_in_diameter, bubble_out_diameter, zoom_factor, zoom_cutoff, image_width = outputsize, num_frames = num_frames)
  
  
  '''
  try:
    one_shot(folder_name, pano_A_filepath, pano_B_filepath, pano_A_zoom_centre, pano_B_zoom_centre, bubble_in_diameter, bubble_out_diameter, zoom_factor, zoom_cutoff, 1808.0, 760.0, image_width = outputsize, num_frames = num_frames)
  except Exception:
    print("Unexpected error:", sys.exc_info()[0])
    print("when generating " + folder_name)
  '''
...
if __name__ == '__main__':
  start_one_shot_generation(4017, 29, './blanford_downstairs/', 'db595', '1d825')
  start_one_shot_generation(1242, 2916, './blanford_downstairs/', 'db595', '6561b')
  start_one_shot_generation(2060, 2061, './blanford_downstairs/', 'e5e6c', '674b3')
  start_one_shot_generation(627, 731, './blanford_downstairs/', 'e5e6c', '4545f')
  start_one_shot_generation(104, 3091, './blanford_downstairs/', 'ed36c', '674b3')